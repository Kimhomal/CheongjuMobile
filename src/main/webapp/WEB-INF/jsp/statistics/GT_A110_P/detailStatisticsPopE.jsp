...
<form  action="${context}/statistics/selectDetailFcltsExcelDown.do" id="frm" name="frm" method="post">
    ...
    <input type="hidden" id="facilityName" name="facilityName" value="신호등" />
    ...
    <input type="hidden" id="mopCde" name="mopCde" value="" />
    <input type="hidden" id="kndCde" name="kndCde" value="" />
    <input type="hidden" id="useCde" name="useCde" value="" />
    <input type="hidden" id="signalCde" name="signalCde" value="" />
    <input type="hidden" id="drnTypeCde" name="drnTypeCde" value="" />
...
<div class="contBox_tp3" style="width: 761px;">
	<div class="area_wrap">
		<div class="area_l w50p pd_r10">
			...
			<div id="mUse" class="board"></div>
			<div id="mKnd" class="board"></div>
			<div id="mDrnType" class="board"></div>
		</div>
		<div class="area_r w50p pd_r10">
			<div id="mDong" class="board"></div>
			<div id="mCtlMet" class="board"></div>
			<div id="mSignalCde" class="board"></div>
			<div id="mMop" class="board"></div>
		</div>
	</div>
...
<script>
    ...
    $(function() {
        ...
        mCtlMet =  new MultiSelectBoard("#mCtlMet", {
        ...
        mMop =  new MultiSelectBoard("#mMop", {
        ...
        mUse =  new MultiSelectBoard("#mUse", {
        ...
        mSignalCde =  new MultiSelectBoard("#mSignalCde", {
        ...
        mDrnType =  new MultiSelectBoard("#mDrnType", {
            url : "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=DRN_TYPE&refCd=A110_P'/>",
            type : "get",
            caption : "*방향구분",
            allSelect : false,
            colNames : [
        ...
        $(".btn_excel").click(function() {
            ...
            $("#statGbn").val(mCtlMet.getSelectVal());
            $("#mopCde").val(mMop.getSelectVal());
            $("#kndCde").val(mKnd.getSelectVal());
            $("#useCde").val(mUse.getSelectVal());
            $("#signalCde").val(mSignalCde.getSelectVal());
            $("#drnTypeCde").val(mSignalCde.getSelectVal());
            $("#arrHeader").val("번호, 관리번호, 경찰서명, 구명, 동명, 지번, 교차로, 구분, 설치일자, 재질, 종류, 용도, 신호등종류, 방향구분, X좌표(EPSG:5181), Y좌표(EPSG:5181)");
            $("#frm").submit();
        });
        ...
        $("#search").click(function() {
            searchResultList.reloadList({
                ...
                statGbn:mCtlMet.getSelectVal(),
                mopCde : mMop.getSelectVal(),
				kndCde : mKnd.getSelectVal(),
				useCde : mUse.getSelectVal(),
				sinalCde : mSignalCde.getSelectVal(),
				drnTypeCde : mDrnType.getSelectVal(),
                stYmd : $("#stYmd").val(),
                enYmd : $("#enYmd").val()
            });
        });
        ...
        searchResultList = new SearchResultList(".detailBoard", {
            ...
            colNames		: [
                ...
                {name : "구분", width : "100px"},
                {name : "설치일자", width : "100px"},
                {name : "재질", width : "100px"},
                {name : "종류", width : "120px"},
                {name : "용도", width : "100px"},
                {name : "신호등종류", width : "100px"},
                {name : "방향구분", width : "100px"},
                {name : "X좌표(EPSG:5181)", width : "100px"},
                {name : "Y좌표(EPSG:5181)", width : "100px"}
            ],
            colModel		: [
                {name : "num"},
                {name : "mgrnu"},
