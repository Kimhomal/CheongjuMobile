...
<form  action="${context}/statistics/selectDetailFcltsExcelDown.do" id="frm" name="frm" method="post">
    ...
    <input type="hidden" id="facilityName" name="facilityName" value="제어기" />
    ...
    <input type="hidden" id="conForCde" name="conForCde" value="" />
	<input type="hidden" id="lineMetCde" name="lineMetCde" value="" />
	<input type="hidden" id="rodKndCde" name="rodKndCde" value="" />
...
<div class="contBox_tp3" style="width: 761px;">
	<div class="area_wrap">
		...
		<div class="area_l w50p pd_r10">
			<div id="mConFor" class="board"></div>
			<div id="mLineMet" class="board"></div>
		</div>
		<div class="area_r w50p pd_r10">
			<div id="mRodKnd" class="board"></div>
		</div>
	</div>
...
<script>
    ...
    $(function() {
        ...
        mConFor =  new MultiSelectBoard("#mConFor", {
        ...
        mLineMet =  new MultiSelectBoard("#mLineMet", {
        ...
        mRodKnd =  new MultiSelectBoard("#mRodKnd", {
        ...
        $(".btn_excel").click(function() {
            ...
            $("#conForCde").val(mConFor.getSelectVal());
            $("#lineMetCde").val(mLineMet.getSelectVal());
            $("#rodKndCde").val(mRodKnd.getSelectVal());
            $("#arrHeader").val("번호, 관리번호, 경찰서명, 구명, 동명, 지번, 교차로, 제어방식, 설치일자, 기종, 제조회사, 제조일자, 계량기관리번호, 계량기제조일자, 신호기제식, 배선방식, 도로구분, X좌표(EPSG:5181), Y좌표(EPSG:5181)");
            $("#frm").submit();
        });
        ...
        $("#search").click(function() {
            searchResultList.reloadList({
                ...
                conForCde:mCtlMet.getSelectVal(),
                lineMetCde:mCtlMet.getSelectVal(),
                rodKndCde:mCtlMet.getSelectVal(),
                stYmd : $("#stYmd").val(),
                enYmd : $("#enYmd").val()
            });
        });
        ...
        searchResultList = new SearchResultList(".detailBoard", {
            ...
            colNames		: [
                ...
                {name : "제어방식", width : "100px"},
                {name : "설치일자", width : "100px"},
                {name : "기종", width : "100px"},
                {name : "제조회사", width : "120px"},
                {name : "제조일자", width : "100px"},
                {name : "계량기관리번호", width : "100px"},
                {name : "계량기제조일자", width : "100px"},
                {name : "신호기제식", width : "100px"},
                {name : "배선방식", width : "100px"},
                {name : "도로구분", width : "100px"},
