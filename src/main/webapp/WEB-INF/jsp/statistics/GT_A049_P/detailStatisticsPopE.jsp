...
<form  action="${context}/statistics/selectDetailFcltsExcelDown.do" id="frm" name="frm" method="post">
    ...
    <input type="hidden" id="facilityName" name="facilityName" value="검지기" />
    ...
    <input type="hidden" id="drTypeCde" name="drTypeCde" value="" />
    <input type="hidden" id="drKndCde" name="drKndCde" value="" />
...
<div class="contBox_tp3" style="width: 761px;">
	<div class="area_wrap">
		...
		<div class="area_l w50p pd_r10">
			<div id="mDrType" class="board"></div>
		</div>
		<div class="area_r w50p pd_r10">
			<div id="mDrKnd" class="board"></div>
		</div>
	</div>
...
<script>
    ...
    $(function() {
        ...
        mCtlMet =  new MultiSelectBoard("#mCtlMet", {
            url : "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=Q_USE&refCd=A049_P'/>",
            type : "get",
            caption : "*용도",
            allSelect : false,
            colNames : [
        ...
        mDrType =  new MultiSelectBoard("#mDrType", {
            url : "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=DR_TYPE&refCd=A049_P'/>",
            type : "get",
            caption : "*검지기유형",
            allSelect : false,
            colNames : [
        ...
        mDrKnd =  new MultiSelectBoard("#mDrKnd", {
            url : "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=DR_KND&refCd=A049_P'/>",
            type : "get",
            caption : "*종류",
            allSelect : false,
            colNames : [
        ...
        $(".btn_excel").click(function() {
            ...
            $("#drTypeCde").val(mDrType.getSelectVal());
            $("#drKndCde").val(mDrKnd.getSelectVal());
            $("#arrHeader").val("번호, 관리번호, 경찰서명, 구명, 동명, 지번, 교차로, 용도, 설치일자, 검지기유형, X좌표(EPSG:5181), Y좌표(EPSG:5181)");
            $("#frm").submit();
        });
        ...
        $("#search").click(function() {
            searchResultList.reloadList({
                ...
                drKndCde:mDrKnd.getSelectVal(),
                drTypeCde : mDrType.getSelectVal(),
                stYmd : $("#stYmd").val(),
                enYmd : $("#enYmd").val()
            });
        });
