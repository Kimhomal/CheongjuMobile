...
<div class="tabCont active">
		...
		<li>
			<div class="tit">구성형식</div>
			<div class="cont">
				<c:out value='${result.formNam}'/>
			</div>
		</li>
		<li>
...
<!-- //압버튼/보행자버튼통합 상세조회 : end-->

<!-- 압버튼/보행자버튼통합 수정 : start-->
<div class="tabCont">
...
$(function(){
	...
	var knd = new AjaxComboBox("#knd", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=KND&refCd=A090_P'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load({select: "${result.knd}"});

	var form1 = new AjaxComboBox("#form1", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=FORM&refCd=A090_P'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load({select: "${result.form}"});
