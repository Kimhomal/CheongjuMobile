...
<div class="tabCont active">
		...
		<li>
			<div class="tit">형식</div>
			<div class="cont">
				<c:out value='${result.crsTypeNam}'/>
			</div>
		</li>
		<li>
			<div class="tit">가로길이(폭)</div>
			<div class="cont">
				<c:out value='${result.hol}'/>
			</div>
		</li>
		<li>
			<div class="tit">신호존재유무</div>
			<div class="cont">
				<c:out value='${result.slYnNam}'/>
			</div>
		</li>
		<li>
...
<div class="tabCont">
		...
		<li>
			<div class="tit">형식</div>
			<div class="cont">
				<select id="crsType" name="crsType">
				</select>
			</div>
		</li>
		<li>
			<div class="tit"><span class="necessary">*</span>가로길이(폭)</div>
			<div class="cont">
				<input type="text" id="hol" name="hol" value="<c:out value='${result.hol}'/>" maxlength="50"/>
			</div>
		</li>
		<li>
			<div class="tit">신호존재유무</div>
			<div class="cont">
				<select id="slYn" name="slYn">
				</select>
			</div>
		</li>
		<li>
		...
		<li>
			<div class="tit">세로길이</div>
			<div class="cont">
				<input type="text" class="w100p readonly" id="hol" name="hol" value="<c:out value='${result.hol}'/>" maxlength="30" readOnly/>
			</div>
		</li>
		<li>
			<div class="tit">X</div>
...
$(function(){
	...
	var crsType = new AjaxComboBox("#crsType", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=CRS_TYPE&refCd=A004_A'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load({select: "${result.crsType}"});

	var slYn = new AjaxComboBox("#slYn", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=SL_YN&refCd=A004_A'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load({select: "${result.slYn}"});
...
// 	$frm.find("#delFileId").val(delFileId.join(","));
	...
	if(confirm("수정하시겠습니까?")) {
		...
		var options = {
			...
			type			: "post",
			data			:{requestYn:$('#requestYn').val(),mapDemandId:$('#mapDemandId').val()},
			dataType		: "json",
			success			:  function(json) {
				if(json.cnt == 1){
					alert("정상적으로 수정되었습니다.");
