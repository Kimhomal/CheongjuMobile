...
<!-- 부착대 등록 : start-->
	...
	<input type="hidden" id="gbn" name="gbn" value="<c:out value='${params.gbn}'/>" />
		...
		<li>
			<div class="tit"><span class="necessary">*</span>길이</div>
			<div class="cont">
				<select id="asnLen" name="asnLen">
				</select>
			</div>
		</li>
		<li>
...
$(function(){
	...
	var knd = new AjaxComboBox("#knd", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=KND&refCd=A057_L'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load();

	var asnLen = new AjaxComboBox("#asnLen", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=ASN_LEN&refCd=A057_L'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load();

	var stat = new AjaxComboBox("#stat", {
		url				: "<c:url value='/common/util/code/combobox.json?codegroup=commonCode&codeId=STAT&refCd=A057_L'/>",
		baseItem	: {name: "--- 선택 ---", value: ""}
	}).load();
...
// 	var ft = mapMaker.layer.polygon.getSource().getFeatures()[0];
	...
	var angle =  mapMaker.mapAction.getAngle(feature.getGeometry().getLastCoordinate(),feature.getGeometry().getFirstCoordinate()); // 시작점, 끝점으로 각도 구함

	var endPoint = mapMaker.mapAction.moveToDistance(coord,$("#asnLen").val(),angle);  // 시작점에서 구한 각도로 길이만큼 이동한 포인트

	console.log("시작점 : " + coord);
	console.log("끝점 : " + endPoint);

	var pt = ol.extent.getCenter(extent);

	var newLineFt = new ol.Feature({
		geometry: new ol.geom.LineString([coord,endPoint])
	});  // 이동한 포인트점으로 새로운 라인 피처 생성

// 	var length = newLineFt.getGeometry().getLength();

	$("#leng").val($("#asnLen").val());

	var geom = mapMaker.getWkt(newLineFt); // 새로 생성한 피처로 wkt 만듬.
