...
/**
	...
	/**
	 * 숫자형식의 문자에서 자리수를 정해놓고 빈자리를 "0"으로 채운다.
	 * 
	 * chgFormat("25", 5) => "00025"
	 * 
	 */
	public static String chgFormat(String oriNo, int no) {
		if( oriNo.length() < no ) {
			while(oriNo.length() < no) {
				oriNo = "0" +oriNo;
			}
		}
	 ...
	 * 현재 날짜를 형식 대로 가져옴
	 * 
	 * getNowDate("yyyy-mm") -> "2003-01"
	 *   -- 년:yyyy, 월:MM, 일:dd, 시간:hh, 분:mm, 초:ss
	 */
	public static String getNowDate(String type) {
		SimpleDateFormat FORMIS = null;
		try {
			FORMIS = new SimpleDateFormat(type);
		}
		catch(Exception ex) {
			FORMIS = new SimpleDateFormat("yyyy-MM-dd");
		}
		return FORMIS.format( new java.util.Date() );
	}
	 ...
	 *   -- 년:yyyy, 월:MM, 일:dd, 시간:hh, 분:mm, 초:ss
	 */
    public static String diffOfMinutes(String begin, String end) throws Exception {
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

        Date beginDate = formatter.parse(begin);
        Date endDate = formatter.parse(end);
        long soyotime = (endDate.getTime() - beginDate.getTime()) / (60 * 1000);
        String result = "";

        if (soyotime > 59) {
            int temphour = (int) (soyotime / 60);
            int tempmin = (int) (soyotime - (temphour * 60));
            String hour = (temphour < 10) ? "0" + String.valueOf(temphour) :
                          String.valueOf(temphour);
            String min = (tempmin < 10) ? "0" + String.valueOf(tempmin) :
                         String.valueOf(tempmin);
            result = hour + ":" + min;
        } else {
            String min = (soyotime < 10) ? "0" + String.valueOf(soyotime) :
                         String.valueOf(soyotime);
            result = "00:" + min;
        }
