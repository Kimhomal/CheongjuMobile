...
public class FreeController {
	...
	//자유게시판 페이지
	@AuthHandler(handler="M008006000")
	@RequestMapping(value = "/board/free/selectFreeList.do")
	public String freePage() throws Exception {
		return "board/free/selectInfoList";
	}

	//자유게시판 조회
	@AuthHandler(handler="M008006000")
	@RequestMapping(value = "/board/free/selectInfoList", method = RequestMethod.POST, produces = {MediaType.APPLICATION_JSON_VALUE})
	public ModelMap selectInfoList(@RequestParam HashMap<String, Object> params, ModelMap model) throws Exception {
		...
		// 리스트조회
		List<?> resultList = freeService.selectInfoList(params);
	...
	@AuthHandler(handler="M008006000")
	@RequestMapping(value = "/board/free/crudInfo.do")
	public String initPage(@RequestParam HashMap<String, Object> params, ModelMap model, HttpServletRequest request) throws Exception {
		...
		if (!"insert".equals(method)) {
			EgovMap result = freeService.selectInfo(params);
			if(Integer.parseInt((result.get("filecnt").toString()))>0){
				List<?> fileList = freeService.selectFileInfo(params);
				params.put("fileYn", "Y");
		...
		ResourceVO vo = (ResourceVO) resources.get("M008006000");

		return "board/free/" + method + "Info";
	}

	@RequestMapping(value = "/board/free/fileDown.do")
	...
	@SuppressWarnings("unchecked")
	@AuthHandler(handler="M008006000")
	@RequestMapping(value = "/board/free/{method}Info", method = RequestMethod.POST, produces = { MediaType.APPLICATION_JSON_VALUE })
	public ModelMap cudInfo(@PathVariable String method, @RequestParam HashMap<String, Object> params, ModelMap model, HttpServletRequest request) throws Exception {
		...
		try {
			if ("insert".equals(method)) {
				...
				int ntMaxId = freeService.selectFreeMaxID(params);
				params.put("freeid", ntMaxId);
				...
				freeService.insertInfo(params, request);
				...
				freeService.updateInfo(params, request);
			} else {
				freeService.deleteInfo(params);
			}
