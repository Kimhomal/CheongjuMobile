...
public class ReferenceController {
	...
	//교통안전시설법령/지침정보 페이지
	@AuthHandler(handler="M008004000")
	@RequestMapping(value = "/board/reference/selectReferenceList.do")
	public String referencePage() throws Exception {
		return "board/reference/selectInfoList";
	}

	//교통안전시설법령/지침정보 조회
	@AuthHandler(handler="M008004000")
	@RequestMapping(value = "/board/reference/selectInfoList", method = RequestMethod.POST, produces = {MediaType.APPLICATION_JSON_VALUE})
	public ModelMap selectInfoList(@RequestParam HashMap<String, Object> params, ModelMap model) throws Exception {
		...
		// 리스트조회
		List<?> resultList = referenceService.selectInfoList(params);
	...
	@AuthHandler(handler="M008004000")
	@RequestMapping(value = "/board/reference/crudInfo.do")
	public String initPage(@RequestParam HashMap<String, Object> params, ModelMap model, HttpServletRequest request) throws Exception {
		...
		if (!"insert".equals(method)) {
			EgovMap result = referenceService.selectInfo(params);
			if(Integer.parseInt((result.get("filecnt").toString()))>0){
				List<?> fileList = referenceService.selectFileInfo(params);
				params.put("fileYn", "Y");
		...
		ResourceVO vo = (ResourceVO) resources.get("M008004000");

		return "board/reference/" + method + "Info";
	}

	@RequestMapping(value = "/board/reference/fileDown.do")
	...
	@SuppressWarnings("unchecked")
	@AuthHandler(handler="M008004000")
	@RequestMapping(value = "/board/reference/{method}Info", method = RequestMethod.POST, produces = { MediaType.APPLICATION_JSON_VALUE })
	public ModelMap cudInfo(@PathVariable String method, @RequestParam HashMap<String, Object> params, ModelMap model, HttpServletRequest request) throws Exception {
		...
		try {
			if ("insert".equals(method)) {
				...
				int ntMaxId = referenceService.selectReferenceMaxID(params);
				params.put("referenceid", ntMaxId);
				...
				referenceService.insertInfo(params, request);
				...
				referenceService.updateInfo(params, request);
			} else {
				referenceService.deleteInfo(params);
			}
